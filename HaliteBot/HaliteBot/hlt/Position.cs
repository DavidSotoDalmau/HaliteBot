...
namespace Halite2.hlt {
    ...
    public class Position {
        ...
        public Position GetClosestPointToAttack(Position target)
        {
            double radius = target.GetRadius() + Constants.MIN_DISTANCE_FOR_CLOSEST_POINT;
            double angleRad = target.OrientTowardsInRad(this);

            double x = target.GetXPos() - 0.5 + radius * Math.Cos(angleRad);
            double y = target.GetYPos() - 0.5 + radius * Math.Sin(angleRad);
